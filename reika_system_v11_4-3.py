"""
ReikaSystem v11.4 — Reference Implementation
=============================================
...
Paper term -> Implementation component mapping:
  ...
  DriftCalibrator             -> EXPERIMENTAL / OPTIONAL (see note below)
...
Language note:
  RF/GM proxy features are English-biased (connectives, capitalization,
  citation markers). High-context environments (e.g. Japanese) require
  recalibration. See paper Section 14 (Limitations).
"""
...
class SeedMatcher:
    def __init__(self, seed_facts=None):
        ...
        self._vecs = [self._vec(f) for f in self.seed_facts]

    def _vec(self, text):
        toks = re.sub(r"[^a-z0-9\s]", "", text.lower()).split()
        d = {}
        for t in toks: d[t] = d.get(t, 0.0) + 1.0
        norm = math.sqrt(sum(v*v for v in d.values())) or 1.0
        return {k: v/norm for k, v in d.items()}

    def _cos(self, a, b): return sum(a.get(k, 0.0)*v for k,v in b.items())
...
class FingerprintMonitor:
    CONN = {"however","therefore","clearly","obviously","certainly","moreover","furthermore","thus","hence","undoubtedly"}
    ASRT = {"must","certainly","clearly","definitely","always","never","absolutely","guaranteed","proven"}
    def __init__(self, window_size=20): self.window_size = window_size
    def analyze(self, tokens):
        n = max(len(tokens), 1); low = [t.lower() for t in tokens]
        cr = sum(1 for t in low if t in self.CONN)/n
        ar = sum(1 for t in low if t in self.ASRT)/n
        er = sum(1 for t in tokens if t and t[0].isupper())/n
        return FingerprintSignal(cr>0.15, ar>0.10, er<0.05, cr>0.10 and ar>0.08)
...
class RiskEngine:
    ...
    def calculate_ht(self, text):
        return float(max(0.0, min(1.0, 1.0 - self.seed.hit_rate(text))))

    def _rf(self, toks):
        CONNECTIVES = {"therefore","thus","hence","consequently","accordingly","moreover","furthermore","additionally","besides","also","however","nevertheless","nonetheless","yet","although","despite","conversely","clearly","obviously","evidently","undoubtedly","certainly","absolutely","definitely","indeed","truly","surely"}
        ASSERTIVENESS = {"must","always","never","impossible","guaranteed","definitive","conclusive","prove","proven","fact","undeniable","irrefutable","unquestionable"}
    ...
    def _gm(self, toks):
        n = max(len(toks), 1)
        entity_count = sum(1 for i, t in enumerate(toks) if t and t[0].isupper() and len(t) > 1 and (i == 0 or toks[i-1][-1] not in '.!?'))
        entity_density = entity_count / n
        citation_count = sum(1 for t in toks if t.startswith("http") or t.startswith("[") or t.startswith("(") or (t.startswith('"') and len(t) > 3))
    ...
    def estimate(self, tokens):
        ...
        rf = self._rf(tokens)
        gm = self._gm(tokens)
        return RiskMetrics(self.calculate_ht(text), self.calculate_lt(rf, gm), rf, gm)
...
class SoftHoldAnnotator:
    ...
    def annotate(self, text):
        ...
        out=[]; warns=[]
        for s in sents:
            m = self.engine.estimate(s.split())
...
class Prism:
    """
    Minimal deterministic post-check — NOT a strong verifier.
    Catches structural failures (empty output, truncation, unbalanced parens,
    missing citation markers on assertive claims).
    ...
    def validate(self, text):
        failed = []
        if len(text.strip()) == 0: failed.append("non_empty")
        if text.strip().endswith("..."): failed.append("no_truncation")
        if text.count("(") != text.count(")"): failed.append("balanced_parens")
        if not re.search(r'http[s]?://|\[|\d+年|\d+%|according to', text.lower()):
            if any(word in text.lower() for word in ["must", "fact", "prove", "certainly"]):
...
class ReikaController:
    def __init__(self, llm, config=None, dev_mode: bool = False):
        ...
        self.calibrator=DriftCalibrator(self.risk); self.recovery=RecoveryProtocol()
    ...
    async def run(self, query, stakes=0.5, irreversible=False):
        ...
        if not safe:
            ...
            self.calibrator.record(a.type); return a
        ...
        async for tok in self.llm.stream(query):
            ...
            if len(buf) % self.config.window_size == 0:
                ...
                fp2=self.fp.analyze(win); m=self.risk.estimate(win)
        ...
        if abort:
            self.state=abort.type.value; self.calibrator.record(abort.type); return abort

        full = " ".join(buf)
        m = self.risk.estimate(buf)
        ...
        self.state=a.type.value; self.calibrator.record(a.type)
        self.calibrator.calibrate(); return a
...
async def demo():
    print("ReikaSystem v11.4 — Reference Implementation Demo")
    print("(Grok修正統合版 / arXiv prototype)")
    print("="*70)
